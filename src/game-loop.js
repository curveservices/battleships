...
const gameLoop = () => {
    ...
    optionShips.forEach(optionShip => optionShip.addEventListener('dragstart', playerBoard.dragStart))
    ...
    function flip() {
        ...
        optionShips.forEach(optionShip => optionShip.style.transform = `rotate(${playerBoard.angle}deg)`);
    };
    ...
    function computerGo() {
        if (!computer.gameOver && !player.gameOver) {
            ...
            setTimeout(() => {
                let randomGo = Math.floor(Math.random() * playerBoard.width * playerBoard.width)
        ...
        function checkShip(shipName, shipLength) {
            if (
                ...
                if (user === 'player') {
                    ...
                    player.hits = userHits.filter(storedShipName => storedShipName !== shipName)
                }
                if (user === 'computer') {
                    infoDisplay.textContent = `The Admiral Bot sunk your ${shipName}`
                    computer.hits = userHits.filter(storedShipName => storedShipName !== shipName)
                }
